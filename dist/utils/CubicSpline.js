...
export class CubicSpline {
    ...
    computeNaturalKs(xs, ys) {
        ...
        for (let i = 1; i < n; i++) {
            ...
            A[i][n + 1] = 3 * ((ys[i] - ys[i - 1]) / (Math.pow((xs[i] - xs[i - 1]), 2)) + (ys[i + 1] - ys[i]) / (Math.pow((xs[i + 1] - xs[i]), 2)));
        }
        A[0][0] = 2 / (xs[1] - xs[0]);
        A[0][1] = 1 / (xs[1] - xs[0]);
        A[0][n + 1] = 3 * (ys[1] - ys[0]) / (Math.pow((xs[1] - xs[0]), 2));
        A[n][n - 1] = 1 / (xs[n] - xs[n - 1]);
        A[n][n] = 2 / (xs[n] - xs[n - 1]);
        A[n][n + 1] = 3 * (ys[n] - ys[n - 1]) / (Math.pow((xs[n] - xs[n - 1]), 2));
