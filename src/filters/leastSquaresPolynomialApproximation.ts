...
import leastSquareSolver from './LeastSquareSolver.js';
...
//     const overlap = 0; // Количество точек перекрытия между сегментами

//     // Разделяем точки на два сегмента с перекрытием
//     const segment1 = points.slice(0, midpointIndex + 1 );
//     const segment2 = points.slice(midpointIndex - overlap);
...
//     // // Одно условие
//     let A_data_enhanced = new Array(2 * (degree + 1) + 1).fill(0).map(() => new Array(2 * (degree + 1) + 1).fill(0));
//     let B_data_enhanced = new Array(2 * (degree + 1) + 1).fill(0);
...
//     // // Дополнительные условия для равенства первых производных в узловой точке
