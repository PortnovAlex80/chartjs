...
import { generateError } from '../sectiongenerators/errorGenerator';
 ...
 */
export function generatePolylineWithVertices(vertices: IVertex[], errorFunction: () => number): IPoint[] {
    const points: IPoint[] = [];
    let currentX = 0;
    let currentY = 0;
    ...
    vertices.forEach(vertex => {
      ...
      for (let i = 0; i < vertex.length; i++) {
        ...
        points.push({ x: currentX, y: currentY } as IPoint);
      }
    });
 ...
 * @returns {IPoint[]} Массив сгенерированных точек полилинии.
 */
export function generatePolyline(): IPoint[] {
